...
export const useItemSearchStore = defineStore("itemSearch", () => {
	...
	/**
	 * Reset navigation after order completion.
	 * Non-Call Center → back to item groups (keep price list)
	 * Call Center → back to price list selection
	 */
	async function resetNavigation(isCallCenter = false) {
		if (isCallCenter) {
			changePriceList()
		} else {
			// Keep selected price list, reset to top-level groups
			groupBreadcrumb.value = []
			selectedItemGroup.value = null
			currentGroupChildren.value = []
			replaceAllItems([])
