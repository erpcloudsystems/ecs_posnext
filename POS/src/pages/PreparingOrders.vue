...
async function getSalesOrder() {
  columns.value = [];
  try {
    const r = await call('ecs_posnext.api.kitchen_order.get_orders', { status: 'Preparing' });
    if (r && r.length > 0) columns.value = r[0];
  } catch (e) { console.error('Error fetching preparing orders:', e); }
}
...
async function onClickHandle(task) {
  ...
  try {
    await call('ecs_posnext.api.kitchen_order.update_item_status', {
      name: so, status: 'Packing', item_row_names: JSON.stringify(item_row_names), time: elapsedSeconds,
    });
...
async function completeAllOrders() {
  ...
  try {
    for (const task of tasks) {
      ...
      const elapsedSeconds = task.creation ? Math.floor((new Date() - new Date(task.creation)) / 1000) : 0;
      await call('ecs_posnext.api.kitchen_order.update_item_status', {
        name: task.name, status: 'Packing', item_row_names: JSON.stringify(item_row_names), time: elapsedSeconds,
      });
    }
    await getSalesOrder();
...
function handleRealtimeUpdate() { getSalesOrder(); }

function handleMsgFallback(data) {
  const event = data?.event || data?.message?.event;
  if (event === 'kitchen_order_preparing') handleRealtimeUpdate();
}
...
function setupSocket() {
  ...
  if (_socket) {
    _socket.connect();
    _socket.on('kitchen_order_preparing', handleRealtimeUpdate);
...
onBeforeUnmount(() => {
  if (_socket) {
    _socket.off('kitchen_order_preparing', handleRealtimeUpdate);
