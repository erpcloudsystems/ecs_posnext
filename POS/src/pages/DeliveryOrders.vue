  ...
  <div class="order-container">
    ...
    <div class="driver-filter-row">
      <div class="driver-filter-wrapper">
        <input v-model="driverSearch" type="text" class="search-input" :placeholder="__('Search Driver...')" @focus="loadDrivers" />
        <div v-if="driverSearch && filteredDriverOptions.length" class="driver-dropdown">
          <div v-for="d in filteredDriverOptions" :key="d.name" class="driver-dropdown-item" @click="selectDriverFilter(d)">
            {{ d.full_name || d.name }}
          </div>
        </div>
      </div>
      <button v-if="selectedDriver" class="btn-clear" @click="clearDriverFilter">✕ {{ __('Clear') }}</button>
      <span v-if="selectedDriver" class="active-filter">{{ selectedDriverName }}</span>
    </div>
      ...
      <div class="modal-card">
        ...
        <div class="modal-body">
          ...
          <table v-else class="items-table">
            ...
            <tbody>
              <tr v-for="item in selectedItems" :key="item.name || item.item_code">
...
async function getSalesOrder() {
  columns.value = [];
  try {
    const r = await call('ecs_posnext.api.kitchen_order.get_orders', { status: 'Delivery' });
    if (r && r.length > 0) columns.value = r[0];
  } catch (e) { console.error('Error fetching delivery orders:', e); }
}
...
async function onClickHandle(task) {
  ...
  try {
    await call('ecs_posnext.api.kitchen_order.update_item_status', {
      name: so, status: 'Completed', item_row_names: JSON.stringify(item_row_names), time: elapsedSeconds,
    });
    if (columns.value.tasks) columns.value.tasks = columns.value.tasks.filter(item => item.name !== so);
...
function handleMsgFallback(data) {
  ...
  if (event === 'kitchen_order_delivery') getSalesOrder();
}
...
function setupSocket() {
  ...
  if (_socket) {
    _socket.connect();
    _socket.on('kitchen_order_delivery', getSalesOrder);
...
onBeforeUnmount(() => {
  if (_socket) {
    _socket.off('kitchen_order_delivery', getSalesOrder);
...
.driver-filter-row { display: flex; align-items: center; gap: 8px; margin-bottom: 12px; }
.driver-filter-wrapper { position: relative; width: 240px; }
.search-input { width: 100%; padding: 8px 12px; border: 1px solid #ccc; border-radius: 8px; font-size: 14px; }
.driver-dropdown { position: absolute; top: 100%; left: 0; right: 0; background: #fff; border: 1px solid #ddd; border-radius: 0 0 8px 8px; max-height: 200px; overflow-y: auto; z-index: 100; box-shadow: 0 4px 8px rgba(0,0,0,0.1); }
.driver-dropdown-item { padding: 8px 12px; cursor: pointer; } .driver-dropdown-item:hover { background: #e3f2fd; }
.btn-clear { background: #eee; border: 1px solid #ccc; border-radius: 6px; padding: 6px 12px; cursor: pointer; font-size: 13px; }
.active-filter { font-weight: 700; color: #0097A7; }
