...
<template>
	<div class="flex flex-col h-full bg-white">
		...
		<div class="px-2.5 py-2 border-b border-gray-200 bg-gray-50">
			<!-- Price list: always uses the POS Profile default (selector removed) -->
			...
			<div ref="customerSearchContainer" class="relative">
				<div v-if="customer">
					...
					<div
						...
						class="mt-2 bg-purple-50 border border-purple-200 rounded-xl p-2"
					>
						<div class="flex items-center justify-between">
							<span class="text-[11px] font-bold text-purple-800">{{ __('Loyalty Reward') }}</span>
							<span v-if="loyaltyStore.customerCategory" class="text-[10px] font-medium text-purple-600">
								{{ loyaltyStore.customerCategory }}<template v-if="loyaltyStore.visits"> · {{ loyaltyStore.visits }} {{ __('visits') }}</template>
							</span>
						</div>
						<div class="grid grid-cols-2 gap-2 mt-1">
							<div>
								<div class="text-[9px] text-gray-500">{{ __('Points') }}</div>
								<div class="text-[12px] font-bold text-purple-700">{{ loyaltyStore.availablePoints.toFixed(2) }}</div>
								<div v-if="loyaltyStore.pointsExpiring > 0 && loyaltyStore.pointsExpiryDate" class="text-[8px] text-amber-600 leading-tight">
									{{ __('{0} expire on {1}', [loyaltyStore.pointsExpiring.toFixed(2), loyaltyStore.pointsExpiryDate]) }}
								</div>
							</div>
							<div>
								<div class="text-[9px] text-gray-500">{{ __('Cashback') }}</div>
								<div class="text-[12px] font-bold text-purple-700">{{ formatCurrency(loyaltyStore.availableCashback) }}</div>
								<div v-if="loyaltyStore.cashbackExpiring > 0 && loyaltyStore.cashbackExpiryDate" class="text-[8px] text-amber-600 leading-tight">
									{{ __('{0} expire on {1}', [loyaltyStore.cashbackExpiring.toFixed(2), loyaltyStore.cashbackExpiryDate]) }}
								</div>
							</div>
						</div>
					</div>

					<!-- Party reservation: Visit Date + Sales Order selector -->
					<ReservationSelector
					...
					<div
						v-if="cartStore.reservationSalesOrder"
						class="mt-2 flex items-center justify-between gap-2 bg-orange-50 border border-orange-200 rounded-lg px-2 py-1.5"
					>
						<span class="text-[10px] font-medium text-orange-700 truncate">
							{{ __("Closing party: {0}", [cartStore.reservationSalesOrder]) }}
						</span>
						<button
							type="button"
							class="text-[10px] font-semibold text-orange-600 hover:text-orange-800"
				...
				<div v-else>
					<div class="flex gap-1.5">
						...
						<!-- Set default customer (VIM ENTERTAINMENT) -->
						<button
							type="button"
							@click="setDefaultVimCustomer"
							class="flex items-center justify-center w-10 h-10 bg-blue-500 hover:bg-blue-600 active:bg-blue-700 rounded-xl text-white transition-colors shadow-sm hover:shadow touch-manipulation flex-shrink-0"
							:title="__('Set VIM Entertainment')"
							:aria-label="__('Set VIM Entertainment')"
						>
							<svg
								class="w-5 h-5"
								fill="none"
...
import ReservationSelector from "./ReservationSelector.vue"
...
// Server-side search (shared customerSearchStore): the field is always usable —
// no full customer preload is required to search.
const customersLoaded = computed(() => true)
...
/**
 * Customer search results — backed by the shared store's server/offline search
 * (no in-memory scan of the full customer list).
 *
 * - <2 chars + focused: show recent customers (store's empty-search list)
 * - >=2 chars: show the fetched matches
 *
 * @returns {Array} Customer objects to show in the dropdown
 */
const customerResults = computed(() => {
	const searchValue = customerSearch.value.trim()

	if (searchValue.length < 2) {
		return customerSearchFocused.value
			? customerSearchStore.filteredCustomers.slice(0, 5)
			: []
	}

	return customerSearchStore.filteredCustomers.slice(0, 20)
})
...
function handleSearchInput(event) {
	...
	// Drive the shared store's server/offline search (debounced inside the store).
	customerSearchStore.setSearchTerm(event.target.value)
}
...
function handleSearchFocus() {
	...
	// Populate recent/browse results for the current term (server/offline).
	customerSearchStore.setSearchTerm(customerSearch.value)
}
...
function selectCustomer(cust) {
	// Track selection for frequent customers feature (pass full object for recent list)
	customerSearchStore.trackCustomerSelection(cust)
...
/**
 * Quick action: set the cart customer to VIM ENTERTAINMENT (the default customer).
 */
function setDefaultVimCustomer() {
	selectCustomer({
		name: "VIM ENTERTAINMENT",
		customer_name: "VIM ENTERTAINMENT",
	})
}
...
/**
 * A reserved party Sales Order was selected: load its items into the cart so the
 * cashier can collect (full - deposit) and close it (reverses the deposit invoice).
 */
function onSelectReservation(salesOrder) {
	cartStore.loadReservationOrder(salesOrder)
}
