					...
					<div class="flex gap-1.5 sm:gap-2">
						...
						<div class="flex-1 min-w-0 flex flex-col justify-center">
							...
							<div class="flex items-start justify-between gap-0.5 mb-0.5">
								...
								<button
									type="button"
									@click.stop="$emit('remove-item', item.item_code, item.uom, item.posa_row_id)"
...
const emit = defineEmits([
	"update-quantity", // (itemCode, newQty, uom?, rowId?) - Update item quantity
	"remove-item", // (itemCode, uom?, rowId?) - Remove item from cart
...
function incrementQuantity(item) {
	...
	emit("update-quantity", item.item_code, newQty, item.uom, item.posa_row_id);
}
...
function decrementQuantity(item) {
	...
	if (newQty <= 0) {
		...
		emit("remove-item", item.item_code, item.uom, item.posa_row_id);
	} else {
		emit("update-quantity", item.item_code, newQty, item.uom, item.posa_row_id);
	}
}
...
function updateQuantity(item, value) {
	...
	if (qty <= 0) return emit("remove-item", item.item_code, item.uom, item.posa_row_id);

	// For positive numbers, update quantity immediately (no rounding here while typing)
	emit("update-quantity", item.item_code, qty, item.uom, item.posa_row_id);
}
...
function handleQuantityBlur(item) {
	...
	if (!item.quantity || item.quantity <= 0) {
		...
		if (roundedQty !== item.quantity) {
			emit("update-quantity", item.item_code, roundedQty, item.uom, item.posa_row_id);
		}
	}
}
