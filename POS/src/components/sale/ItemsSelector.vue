		...
		<div v-else-if="navigationStep === 'groups'" class="flex-1 overflow-y-auto p-3 sm:p-4">
			...
			<div v-else class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
				...
				>
					...
					<!-- Group Name -->
					<span class="text-[11px] sm:text-xs font-semibold text-gray-800 text-center leading-tight">{{ groupLabel(group) }}</span>
			...
			>
				<div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2 sm:gap-3">
					...
					>
						...
						<!-- Item Image -->
						<div class="relative w-full aspect-square mx-auto bg-gray-100 rounded-lg mb-1.5 sm:mb-2 overflow-hidden">
...
// Group display name: Arabic custom name when the POS is in Arabic, else the
// translated group name.
function groupLabel(group) {
	if (locale.value === "ar" && group && group.item_group_arabic) return group.item_group_arabic
	return window.__ ? window.__(group.item_group) : group.item_group
}
...
// Filter out Talabat price list for non-Call Center profiles
const displayPriceLists = computed(() => {
	if (props.posProfile === 'Call Center') return availablePriceLists.value
	return availablePriceLists.value.filter(pl => pl.name !== 'Talabat')
})
