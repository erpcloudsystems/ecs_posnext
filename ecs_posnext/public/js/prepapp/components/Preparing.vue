<template>
  <div class="order-container">
    <div class="top-bar">
      ...
      <h3 class="page-title">{{ __("Preparing") }}</h3>
  ...
  setup() {
    ...
    function handleClickSalesOrder(sales_order) {
      ...
      frappe.call({
        ...
        callback: (r) => {
          if (!r.exc) { item_selected.value = r.message[1]; item_loading.value = false; }
        },
      });
    }
    ...
    function onClickHandle(task) {
      ...
      frappe.call({
        ...
        args: { name: so, status: 'Packing', item_row_names: JSON.stringify(item_row_names), time: elapsedSeconds },
        callback: () => { columns.value.tasks = columns.value.tasks.filter(item => item.name !== so); },
      });
    }
    ...
    async function completeAllOrders() {
      ...
      try {
        for (const task of tasks) {
          ...
          await frappe.call({
            method: 'posawesome.posawesome.api.kitchen_order.update_item_status',
            args: { name: task.name, status: 'Packing', item_row_names: JSON.stringify(item_row_names), time: elapsedSeconds },
          });
        }
        get_sales_order();
    ...
    function getTimerClass(task) {
      ...
      if (s > 600) return 'timer-red'; if (s > 300) return 'timer-orange'; return 'timer-green';
    }
    function getTopColor(task) {
      const t = task.custom_so_type;
      if (t === 'Delivery') return '#E53A40'; if (t === 'Pickup' || t === 'Pick Up') return '#379392'; if (t === 'Car Service') return '#6AAFE6'; return '#FFBC42';
    }
    function getTypeBadgeStyle(task) {
      const t = task.custom_so_type;
      if (t === 'Delivery') return 'color:#E53A40;background:#fde4e4'; if (t === 'Pickup' || t === 'Pick Up') return 'color:#379392;background:#d6f1ec'; if (t === 'Car Service') return 'color:#6AAFE6;background:#e0f0fb'; return '';
    }

    function handleRealtimeUpdate() { get_sales_order(); }

    onMounted(() => {
      get_sales_order();
      frappe.realtime.on('order_preparing', handleRealtimeUpdate);
      elapsedTimeInterval = setInterval(() => { currentTime.value = new Date(); }, 1000);
    });
    onBeforeUnmount(() => {
      frappe.realtime.off('order_preparing', handleRealtimeUpdate);
      if (elapsedTimeInterval) clearInterval(elapsedTimeInterval);
    });

    return { columns, completingAll, itemDetails, item_selected, item_loading, filteredTasks, backToInvoices, handleClickSalesOrder, onClickHandle, completeAllOrders, getElapsedTime, getTimerClass, getTopColor, getTypeBadgeStyle };
  },
};
</script>
