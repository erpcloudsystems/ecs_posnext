...
from ecs_posnext.api.m_pesa import submit_mpesa_payment
...
def create_pos_payment_entry(payload):
    ...
    if invoice_doc.custom_table_number and invoice_doc.custom_so_type == "Dinin":
        ...
        frappe.db.set_value('Table Number', invoice_doc.custom_table_number, "status", "Active")
    # validate data
    ...
    for payment_method in data.payment_methods:
        ...
        if data.get("submit") or not is_call_center:
            ...
            for invoice in all_invoices_list:
                if invoice.get("name"):
                    ...
                    if inv.custom_table_number and inv.custom_so_type == "Dinin":
                        frappe.db.set_value('Table Number', inv.custom_table_number, "status", "Active")
