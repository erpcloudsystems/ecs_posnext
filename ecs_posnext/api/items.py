...
def get_combo_components(item_code, pos_profile=None, price_list=None):
	...
	try:
		...
		for c in components:
			...
			# Parse locked_attributes
			locked_attributes = []
			if getattr(c, "locked_attributes", None):
				try:
					locked_attributes = json.loads(c.locked_attributes)
				except:
					pass
			...
			sections[sn]["options"].append({
				...
				"lock_group": getattr(c, "lock_group", "") or "",
			})
...
def get_item_groups(pos_profile):
	...
	try:
		...
		if not configured_groups:
			result = (
				...
				.select(
					...
					ItemGroup.custom_item_group_arabic.as_("item_group_arabic"),
				)
		...
		# Arabic display names for the configured groups (custom field).
		ar_map = {
			r.name: r.custom_item_group_arabic
			for r in frappe.get_all(
				"Item Group",
				filters=[["name", "in", configured_groups]],
				fields=["name", "custom_item_group_arabic"],
			)
		}
		...
		for group_name in configured_groups:
			...
			result.append({
				...
				"item_group_arabic": ar_map.get(group_name),
...
@frappe.whitelist()
def get_variant_attributes(item_codes):
	"""Return Item Variant Attribute rows for the given item codes (for POS product editor)."""
	...
	rows = frappe.db.sql(
		"""
		SELECT parent, attribute, attribute_value AS value
