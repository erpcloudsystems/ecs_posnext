...
frappe.ui.form.on("Packaging Supplies Request", {
	refresh(frm) {
		if (frm.doc.docstatus === 0) {
			frm.dashboard.set_headline(
				__("Packaging & Supplies Request — flagged items load automatically; enter the request quantity for what you need.")
			);
			frm.add_custom_button(__("Load Packaging & Supplies"), () =>
				load_requisition_items(frm, "ecs_posnext.api.branch_requisition.get_packaging_items")
			);
		}
	},
	...
	onload(frm) {
		if (frm.is_new() && !(frm.doc.items || []).length) {
			load_requisition_items(frm, "ecs_posnext.api.branch_requisition.get_packaging_items", { silent: true });
		}
	},
});
