  ...
  <v-container fluid>
    <v-card flat class="cards mb-2 mt-2 py-0" style="max-height: 11vh; height: 11vh">
      <v-row align="start" no-gutters>
        <v-col cols="3">
          <v-btn block class="pa-1" large color="warning" dark @click="back_to_invoices">
            {{ __("Back") }}
          </v-btn>
        </v-col>
      </v-row>
      ...
      <v-card>
        ...
        <v-container>
          <v-autocomplete v-model="selectedDriver" :items="drivers" item-title="name" item-value="id"
            label="Select Driver" outlined dense></v-autocomplete>
        </v-container>
        <v-card-actions>
          <v-spacer></v-spacer>
      ...
      <div>
        <v-card class="selection mx-auto grey lighten-5" style="max-height: 80vh; height: 80vh">
          ...
          <div class="my-0 py-0 overflow-y-auto" style="max-height: 75vh">
            <template>
              <v-container>
                <v-data-table v-model="expanded" :headers="headers" :items="item_selected" :single-expand="singleExpand"
                  :expanded.sync="expanded" :loading="item_loading" show-expand item-key="posa_row_id"
                  ...
                  <template v-slot:expanded-item="{ headers, item }">
                    <td :colspan="headers.length" class="ma-0 pa-0">
                      <v-data-table :headers="subHeaders" :items="item.items" dense :loading="item_loading"
                        hide-default-footer>
                      </v-data-table>
                    </td>
                  </template>
    ...
    <div class="kanban-board">
      <div class="row columns" style="margin: 0">
        <v-card v-if="!itemDetails" v-for="task in filteredTasks" :key="task.id" outlined style="
...
export default {
  ...
  data() {
    return {
      ...
      expanded: [],
      headers: [
        { text: "Item", value: "parent_item_code" },
        { text: "Qty", value: "qty" },
      ],
      subHeaders: [
        { text: "Item", value: "item_code" },
        { text: "Qty", value: "qty" },
      ],
      loading: false,
      pos_profile: "",
