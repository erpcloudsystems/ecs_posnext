        ...
        >
          ...
          >
            ...
            <span style="font-weight: 500; font-size: 14px; color: gray">
              {{ task.driver }} {{ task.driver_cell_number }}
            </span>
          </div>
          <v-divider></v-divider>
          ...
          >
            ...
            <button
              style="
                ...
                padding: 2px 10px;
              "
              v-if="task.status_name == 'Delivery'"
              @click="
                onClickHandle(
                  task.name,
                  task.custom_so_type,
...
export default {
  ...
  methods: {
    ...
    handleRealtimeUpdate(data) {
      ...
      if (!existingTask) {
        ...
        this.columns.tasks.push({
          ...
          driver: data.driver,
          name: data.order_id,
          driver_cell_number: data.driver_cell_number,
    ...
    get_sales_order() {
      const args = {
        status: "Delivery",
        fromDelivery: true, // Pass this flag to the server method
      };
        ...
        callback: (r) => {
          if (!r.exc) {
            if (r.message.length > 0) {
              const allTasks = r.message.flatMap((status) => status.tasks);
              this.columns = { name: "Delivery", tasks: allTasks };
