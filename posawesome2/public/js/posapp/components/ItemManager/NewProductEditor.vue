<template>
  <v-container fluid class="new-product-editor pa-0 rtl-scope" dir="rtl">
            ...
            <div class="section-box">
              ...
              <div class="section-box__body">
                ...
                ">
                  <div v-for="(group, groupName) in groupedVariants" :key="groupName" class="variant-group mb-3">
                    ...
                    <div class="variant-group__items">
                      <div v-for="(variant, vIndex) in group" :key="variant._uid || vIndex" class="variant-row variant-row--nested">
                        <v-text-field :value="variant.name" @input="variant.name = $event" outlined dense hide-details
                        ...
                        <v-tooltip bottom>
                          <template v-slot:activator="{ on }">
                            <v-btn icon x-small v-on="on" @click="openVariantPricesDialog(variant)"
                              :color="getVariantPricesCount(variant) > 0 ? 'success' : 'grey'">
                              <v-icon x-small>mdi-currency-usd</v-icon>
                            </v-btn>
                          </template>
                          <span>{{ __("الأسعار") }} ({{ getVariantPricesCount(variant) }})</span>
                        </v-tooltip>
                        <v-tooltip bottom>
                          <template v-slot:activator="{ on }">
                ...
                <template v-else-if="product.variants.length > 0">
                  <div v-for="(variant, index) in product.variants" :key="variant._uid || index" class="variant-row">
                    <v-text-field :value="variant.name" @input="variant.name = $event" outlined dense hide-details :placeholder="__('اسم المتغير')"
                    ...
                    <v-tooltip bottom>
                      <template v-slot:activator="{ on }">
                        <v-btn icon small v-on="on" @click="openVariantPricesDialog(variant)"
                          :color="getVariantPricesCount(variant) > 0 ? 'success' : 'grey'">
            ...
            <div class="combo-section__options">
              <div v-for="(option, oIndex) in section.options" :key="oIndex" class="option-item">
                ...
                <div class="option-row" :class="{ 'option-row--template': option.has_variants }">
                  ...
                  <v-tooltip bottom v-if="!option.has_variants">
                    <template v-slot:activator="{ on }">
                      <v-btn icon x-small v-on="on" @click="openExtraPricesDialog(option)"
                        :color="getExtraPricesCount(option) > 0 ? 'success' : 'grey'" class="mx-1">
                        <v-icon x-small>mdi-currency-usd</v-icon>
                      </v-btn>
                    </template>
                    <span>{{ __("الأسعار الإضافية") }} ({{ getExtraPricesCount(option) }})</span>
                  </v-tooltip>
                ...
                " class="nested-variants">
                  <div v-for="(variant, vIndex) in option.variants" :key="vIndex" class="option-row option-row--nested">
                    ...
                    <v-tooltip bottom>
                      <template v-slot:activator="{ on }">
                        <v-btn icon x-small v-on="on" @click="openExtraPricesDialog(variant)"
                          :color="getExtraPricesCount(variant) > 0 ? 'success' : 'grey'" class="mx-1">
                          <v-icon x-small>mdi-currency-usd</v-icon>
                        </v-btn>
                      </template>
                      <span>{{ __("الأسعار الإضافية") }} ({{ getExtraPricesCount(variant) }})</span>
                    </v-tooltip>
    ...
    <v-dialog v-model="showVariantPricesDialog" max-width="500px">
      <v-card>
        <v-card-title>
          <v-icon right color="primary">mdi-currency-usd</v-icon>
          {{ __("أسعار المتغير:") }} {{ currentVariantForPrices ? currentVariantForPrices.name : '' }}
        </v-card-title>
        <v-card-text>
          <div v-if="currentVariantPrices.length === 0" class="text-center grey--text pa-4">
            {{ __("اضغط + لإضافة سعر") }}
          </div>
          <div v-for="(priceItem, idx) in currentVariantPrices" :key="idx" class="d-flex align-center mb-2">
            <v-autocomplete 
              v-model="priceItem.price_list" 
              :items="priceLists" 
              item-text="name" 
              item-value="name"
              outlined 
            ...
            ></v-text-field>
            <v-btn icon x-small color="error" @click="removeVariantPriceRow(idx)" class="ms-1">
              <v-icon small>mdi-close</v-icon>
            </v-btn>
          </div>
          <v-btn text small color="primary" @click="addVariantPriceRow" class="mt-2">
            <v-icon right small>mdi-plus</v-icon>
            {{ __("إضافة سعر") }}
          </v-btn>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn text @click="showVariantPricesDialog = false">{{ __("إلغاء") }}</v-btn>
          <v-btn color="primary" @click="saveVariantPrices" :loading="saving">{{ __("حفظ الأسعار") }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>

    <!-- Extra Prices Dialog (for Combo options) -->
    <v-dialog v-model="showExtraPricesDialog" max-width="500px">
      <v-card>
        <v-card-title>
          <v-icon right color="primary">mdi-currency-usd</v-icon>
          {{ __("الأسعار الإضافية:") }} {{ currentOptionForExtraPrices ? currentOptionForExtraPrices.item_name : '' }}
        </v-card-title>
        <v-card-text>
          <div v-if="currentExtraPrices.length === 0" class="text-center grey--text pa-4">
            {{ __("اضغط + لإضافة سعر إضافي") }}
          </div>
          <div v-for="(priceItem, idx) in currentExtraPrices" :key="idx" class="d-flex align-center mb-2">
            <v-autocomplete 
              v-model="priceItem.price_list" 
              :items="priceLists" 
              item-text="name" 
            ...
            ></v-autocomplete>
            <v-text-field 
              v-model.number="priceItem.extra_price" 
              type="number" 
              outlined 
              dense 
              hide-details 
              ...
              style="max-width: 120px"
            ></v-text-field>
            <v-btn icon x-small color="error" @click="removeExtraPriceRow(idx)" class="ms-1">
              <v-icon small>mdi-close</v-icon>
            </v-btn>
          </div>
          <v-btn text small color="primary" @click="addExtraPriceRow" class="mt-2">
            <v-icon right small>mdi-plus</v-icon>
            {{ __("إضافة سعر") }}
          </v-btn>
        </v-card-text>
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn text @click="showExtraPricesDialog = false">{{ __("إلغاء") }}</v-btn>
          <v-btn color="primary" @click="saveExtraPrices">{{ __("حفظ") }}</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      // Variant Prices Dialog
      showVariantPricesDialog: false,
      currentVariantForPrices: null,
      currentVariantPrices: [], // { price_list: '', price: 0, name: '' }

      // Extra Prices Dialog (for Combo options)
      showExtraPricesDialog: false,
      currentOptionForExtraPrices: null,
      currentExtraPrices: [], // { price_list: '', extra_price: 0 }
    };
  },
    ...
    groupedVariants() {
      ...
      this.product.variants.forEach((variant, index) => {
        ...
        if (variant.attributes && variant.attributes.length > 0) {
          ...
          if (firstAttrObj) {
            groupKey = firstAttrObj.value;
          }
        }
        
        // Use cached _groupKey for variants without attributes (to prevent re-grouping on name change)
        if (!groupKey) {
          if (variant._groupKey) {
            groupKey = variant._groupKey;
          } else {
            // Extract from name only once and cache it
            groupKey = variant.name ? variant.name.split(" - ")[0] : `New-${index}`;
            variant._groupKey = groupKey;
          }
        }
  ...
  methods: {
    ...
    // Variant Prices Methods
    getVariantPricesCount(variant) {
      return (variant.prices || []).length;
    },

    async openVariantPricesDialog(variant) {
      this.currentVariantForPrices = variant;
      
      // Initialize prices array if not exists
      if (!variant.prices) {
        variant.prices = [];
      }
      
      // If variant exists in DB, load its prices
      if (variant.item_code && variant.isExisting) {
        try {
          const res = await frappe.call({
            method: "frappe.client.get_list",
            args: {
          ...
          this.currentVariantPrices = (res.message || []).map((p) => ({
            name: p.name,
            price_list: p.price_list,
            price: p.price_list_rate,
          }));
        } catch (e) {
          console.error("Error loading variant prices:", e);
          this.currentVariantPrices = [...variant.prices];
        }
      } else {
        // For new variants, use the cached prices
        this.currentVariantPrices = [...variant.prices];
      }
      
      this.showVariantPricesDialog = true;
    },

    addVariantPriceRow() {
      this.currentVariantPrices.push({ price_list: "", price: 0 });
    },

    removeVariantPriceRow(idx) {
      this.currentVariantPrices.splice(idx, 1);
    },

    async saveVariantPrices() {
      if (!this.currentVariantForPrices) return;
      
      // Save to variant object
      this.currentVariantForPrices.prices = [...this.currentVariantPrices];
      
      // If variant exists in DB, save to Item Price
      if (this.currentVariantForPrices.item_code && this.currentVariantForPrices.isExisting) {
        try {
          for (const priceItem of this.currentVariantPrices) {
            if (!priceItem.price_list) continue;
            ...
            if (priceItem.name) {
              ...
              const res = await frappe.call({
                ...
                args: {
                  doc: {
                    doctype: "Item Price",
                    item_code: this.currentVariantForPrices.item_code,
                    price_list: priceItem.price_list,
                    price_list_rate: priceItem.price,
                    selling: 1,
                  },
              ...
              // Update the name for future updates
              priceItem.name = res.message.name;
            }
          }
          this.showMessage(this.__("تم حفظ الأسعار بنجاح"));
        } catch (e) {
          console.error("Error saving variant prices:", e);
          this.showMessage(e.message || this.__("خطأ في حفظ الأسعار"), "error");
        }
      } else {
        this.showMessage(this.__("تم حفظ الأسعار (سيتم تطبيقها عند حفظ المنتج)"));
      }
      
      this.showVariantPricesDialog = false;
    },

    // Extra Prices Methods (for Combo options)
    parseExtraPrices(extraPricesStr) {
      if (!extraPricesStr) return [];
      try {
        return JSON.parse(extraPricesStr);
      } catch (e) {
        return [];
      }
    },

    getExtraPricesCount(option) {
      return (option.extra_prices || []).length;
    },

    openExtraPricesDialog(option) {
      this.currentOptionForExtraPrices = option;
      
      // Initialize extra_prices array if not exists
      if (!option.extra_prices) {
        option.extra_prices = [];
      }
      
      // Copy to current for editing
      this.currentExtraPrices = option.extra_prices.map(p => ({ ...p }));
      
      this.showExtraPricesDialog = true;
    },

    addExtraPriceRow() {
      this.currentExtraPrices.push({ price_list: "", extra_price: 0 });
    },

    removeExtraPriceRow(idx) {
      this.currentExtraPrices.splice(idx, 1);
    },

    saveExtraPrices() {
      if (!this.currentOptionForExtraPrices) return;
      
      // Save to option object
      this.currentOptionForExtraPrices.extra_prices = this.currentExtraPrices.filter(p => p.price_list);
      
      // Also set the default extra_price to the first price for backward compatibility
      if (this.currentOptionForExtraPrices.extra_prices.length > 0) {
        this.currentOptionForExtraPrices.extra_price = this.currentOptionForExtraPrices.extra_prices[0].extra_price;
      }
      
      this.showExtraPricesDialog = false;
      this.showMessage(this.__("تم حفظ الأسعار الإضافية"));
    },
    ...
    generateAllVariants() {
      ...
      const existingVariants = [...this.product.variants];
      const newVariants = [];
      ...
      for (const combo of combinations) {
        ...
        const existing = existingVariants.find((v) => {
          ...
          if (v.attributes && v.attributes.length === attrs.length) {
            return attrs.every((a) =>
              v.attributes.some(
                (va) =>
                  va.attribute === a.attribute &&
                  (va.value === a.value || va.attribute_value === a.value),
              ),
            );
          }
          return false;
        ...
        if (existing) {
          // Keep existing variant with its item_code, price, isExisting, etc.
    ...
    async loadProduct() {
      ...
      if (res.message) {
        ...
        if (item.has_variants) {
          await this.loadAttributesAndVariants();
        }
      ...
      components.forEach((comp) => {
        ...
        if (templateCode && !isStandalone) {
          ...
          templateGroups[key].variants.push({
            ...
            extra_prices: this.parseExtraPrices(comp.extra_prices),
    ...
    async loadAttributesAndVariants() {
      try {
        const res = await frappe.call({
          method:
            "posawesome.posawesome.custom_api.item_manager.get_item_variants_with_attributes",
          args: { item_code: this.product.item_code },
        });

        if (res.message && res.message.success) {
          this.product.attributes = res.message.attributes || [];

          this.product.variants = (res.message.variants || []).map((v, idx) => ({
            _uid: v.item_code || `existing-${idx}`,
            item_code: v.item_code,
            name: v.item_name.replace(this.product.item_name + " - ", ""),
            price: v.standard_rate || 0,
            show_bundle: v.custom_show_bundle_in_pos === 1,
            isExisting: true,
            prices: [], // Will be loaded on demand when opening prices dialog
          }));
        }
      } catch (e) {
        console.error("Error loading attributes and variants:", e);
    ...
    async saveStandardWithVariants() {
      ...
      if (!this.isEditing) {
        ...
        // Update template item with all fields
        await frappe.call({
          method: "frappe.client.set_value",
          args: {
            doctype: "Item",
      ...
      for (const variant of this.product.variants) {
        ...
        if (variant.isExisting) {
          ...
          // Ensure all attribute values exist before creating variant
          for (const attr of variant.attributes) {
            await this.ensureAttributeExists(attr.attribute, [attr.value]);
            // Also ensure the attribute is on the template item
            await this.ensureAttributeOnTemplate(attr.attribute);
          }
    ...
    async ensureAttributeOnTemplate(attrName) {
      // Ensure the attribute is added to the template item's attributes list
      if (!this.product.item_code) return;

      try {
        const templateDoc = await frappe.call({
          method: "frappe.client.get",
          args: { doctype: "Item", name: this.product.item_code },
        });

        const template = templateDoc.message;
        const existingAttrs = (template.attributes || []).map(
          (a) => a.attribute,
        );

        if (!existingAttrs.includes(attrName)) {
          template.attributes = template.attributes || [];
          template.attributes.push({ attribute: attrName });
        ...
        console.error("Error ensuring attribute on template:", e);
      }
    },
        ...
        for (const option of section.options) {
          ...
          if (option.has_variants && option.variants) {
            for (const variant of option.variants) {
              if (variant.item_code) {
                components.push({
                  ...
                  extra_prices: JSON.stringify(variant.extra_prices || []),
                  qty: variant.qty || 1,
            ...
            components.push({
              ...
              extra_prices: JSON.stringify(option.extra_prices || []),
              qty: option.qty || 1,
