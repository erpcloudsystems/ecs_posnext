export default {
    ...
    methods: {
        ...
        formtFloat (value, precision, no_truncate = false) {
            const format = get_number_format(this.pos_profile.currency);
            if(no_truncate)
        ...
        setFormatedFloat (el, field_name, precision, no_truncate= false, no_negative = false, $event) {
            let value = 0;
            try {
                // make sure it is a number and positive
                ...
                value = this.formtFloat($event, precision, no_truncate);
            } catch (e) {
                console.error(e);
                value = 0;
            }
