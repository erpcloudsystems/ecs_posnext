...
from frappe.utils import flt, add_days, get_datetime, now_datetime, format_datetime
...
from posawesome.posawesome.api.posapp import (
    get_company_domain,
    get_shift_window_for_opening_shift,
...
def validate(doc, method):
    # validate_shift(doc)
    set_patient(doc)
    if doc.custom_so_type == "Delivery":
        auto_set_delivery_charges(doc)
        calc_delivery_charges(doc)
    if doc.is_return == 1:
        doc.update_billed_amount_in_sales_order = 1
        doc.update_outstanding_for_self = 0
    if doc.custom_table_number and doc.custom_so_type == "Dinin":
        for row in doc.custom_numbers_of_table:
            
            frappe.db.set_value('Table Number', row.table_name,"disabled", 1)
...
def before_submit(doc, method):
    ...
    # if doc.custom_table_number and doc.custom_so_type == "Dinin":
    #     frappe.db.set_value('Table Number', doc.custom_table_number,"disabled", 0)

def after_submit(doc, method):
    pass
    # if doc.is_return and doc.docstatus == 1:
    ...
    #     # Accounts
    #     pe.paid_to = "1310 - مدينون - B"   # ← Change to your cash account
    #     pe.paid_from = "1111 - صندوق نقدي الكاشير - B"   # ← Change to your cash account
    #     pe.mode_of_payment = "صندوق الكاشير - نقدي"

    #     refund_amount = abs(doc.rounded_total)
    #     pe.paid_amount = refund_amount
    #     pe.received_amount = refund_amount

    #     # --- Fix for Source Exchange Rate ---
    #     # Set the customer currency and company currency automatically
    #     # pe.paid_to_account_currency = frappe.get_value("Account", pe.paid_to, "account_currency")
    #     # pe.company_currency = frappe.get_value("Company", doc.company, "default_currency")

    #     # If currency is different, set exchange rate
    #     # if pe.paid_to_account_currency != pe.company_currency:
    #     #     # Get latest exchange rate
    #     #     rate = frappe.get_last_doc("Currency Exchange",
    #     #                             filters={"from_currency": pe.paid_to_account_currency,
    #     #                                         "to_currency": pe.company_currency})
    #     #     pe.source_exchange_rate = rate.exchange_rate
    #     # else:
    #     pe.source_exchange_rate = 1

    #     # Link refund to return invoice
...
def get_last_open_opening_shift(pos_profile):
    ...
    return shift[0] if shift else None

def validate_shift(doc):
    pos_opening_shift = get_last_open_opening_shift(doc.pos_profile)
    if pos_opening_shift:
        doc.posa_pos_opening_shift = pos_opening_shift.name
    ...
    if doc.posa_pos_opening_shift and doc.pos_profile and doc.is_pos:
        ...
        # If profile allows all-day sales, skip window enforcement
        if getattr(shift, "posa_allow_sales_whole_day", None) or getattr(
            frappe.get_cached_doc("POS Profile", shift.pos_profile), "posa_allow_sales_whole_day", 0
        ):
            return

        window = get_shift_window_for_opening_shift(shift)
        if not window:
            frappe.throw(
                _(
                    "POS Shift {0} is outside allowed shift windows. Please close it and open the next shift."
                ).format(shift.name)
            )

        now_dt = now_datetime()
        if now_dt > window.end:
            frappe.throw(
                _(
                    "POS Shift {0} ended at {1}. Please close it and open the next shift."
                ).format(shift.name, format_datetime(window.end))
            )

        posting_time = getattr(doc, "posting_time", None) or "00:00:00"
        posting_dt = get_datetime(f"{doc.posting_date} {posting_time}") if doc.posting_date else now_dt
        if posting_dt < window.start or posting_dt > window.end:
            frappe.throw(
                _(
                    "Invoice posting time is outside the active shift window ({0} to {1}). Adjust the time or close/open the shift."
                ).format(format_datetime(window.start), format_datetime(window.end))
            )
