...
@frappe.whitelist()
def get_item_variants_with_attributes(item_code):
    """
    Get all variants of a template item with their attributes in a single call.
    ...
    try:
        ...
        # Get all variant attributes in one query
        ...
        for attr in all_attrs:
            if attr.attribute not in attr_map:
                attr_map[attr.attribute] = {"attribute": attr.attribute, "values": []}
            if attr.attribute_value and attr.attribute_value not in attr_map[attr.attribute]["values"]:
                attr_map[attr.attribute]["values"].append(attr.attribute_value)
        ...
        frappe.log_error(frappe.get_traceback(), "Get Item Variants With Attributes Error")
        return {"success": False, "message": str(e)}
